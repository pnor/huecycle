...
Assumes testing enviroment has defined `huecycle-counter' to track current
iteration number."
...
(defun huecycle-get-face-color (spec face)
  "Return hex string of color of FACE based on SPEC from `face-remapping-alist'."
  (let ((remaps
         (seq-some
          (lambda (remaps) (if (equal face (nth 0 remaps)) remaps))
          face-remapping-alist)))
    (seq-some (lambda (item)
                (if (and (listp item) (equal spec (nth 0 item))) (nth 1 item)))
              remaps)))
...
HUECYCLE-ITERATIONS controls how many loop iterations `huecycle' does. If it is
 <= 0, it will loop forever."
  `(let ((face-remapping-alist '())
     ...
     (cl-letf
         (((symbol-function #'huecycle--input-pending)
           (apply-partially #'huecycle--test-input-pending huecycle-iterations))
...
(ert-deftest huecycle-cleans-up ()
  ...
  ;; Test huecycle cleans up after itself when ending `huecycle-cycle-duration'
  ;; is positive.
  (huecycle-with-test-env
   (progn
     (huecycle)
  ...
  (huecycle-with-test-env
   (progn
     ...
     (should (equal '() face-remapping-alist)))
   :huecycle--interpolate-data
   (mapcar #'huecycle--init-interp-datum
           '(((foreground . default))
...
(ert-deftest huecycle-uses-random-colors ()
  ...
  (let ((test-colors-list '("#000001" "#000002" "#000003" "#000004")))
    (huecycle-with-test-env
     (progn
       (huecycle)
       (should
        (member (huecycle-get-face-color :foreground 'default) test-colors-list)))
     :huecycle--interpolate-data
     (mapcar (lambda (args) (apply #'huecycle--init-interp-datum args))
             `((((foreground . default))
                :persist t
  ...
  (let ((test-colors-list '("#000001" "#000002" "#000003" "#000004" "#000005")))
    (huecycle-with-test-env
     ...
     :huecycle-step-size 1
     :huecycle-iterations 5))
  (let ((test-colors-list '("#000001" "#000002" "#000003"
                            "#000004" "#000005" "#000006")))
    (huecycle-with-test-env
     (progn
       (huecycle)
       (should (member
                (huecycle-get-face-color :foreground 'highlight)
                test-colors-list))
       (should (member
                (huecycle-get-face-color :foreground 'font-lock-constant-face)
                test-colors-list))
       (should (member
                (huecycle-get-face-color :foreground 'default)
                test-colors-list))
       (should (member
                (huecycle-get-face-color :foreground 'font-lock-keyword-face)
                test-colors-list))
       (should (member
                (huecycle-get-face-color :background 'font-lock-keyword-face)
                test-colors-list))
       (should (member
                (huecycle-get-face-color :background 'font-lock-builtin-face)
                test-colors-list))
       (should (member
                (huecycle-get-face-color :background 'highlight)
                test-colors-list))
       (should (member
                (huecycle-get-face-color :background 'font-lock-warning-face)
                test-colors-list))
       (should (member
                (huecycle-get-face-color :background 'font-lock-string-face)
                test-colors-list))
       (should (member
                (huecycle-get-face-color :distant-foreground
                                         'font-lock-string-face)
                test-colors-list))
       (should (member
                (huecycle-get-face-color :distant-foreground 'default)
                test-colors-list)))
     :huecycle--interpolate-data
     (mapcar (lambda (args) (apply #'huecycle--init-interp-datum args))
             `((((foreground . (highlight font-lock-constant-face)))
...
(ert-deftest huecycle-test-active-buffers-size ()
  "Test validity and ordering of buffers in `huecycle--active-buffers'."
  (huecycle-with-test-env
  ...
  (huecycle-with-test-env
   (let* ((buffer-1 (get-buffer-create "huecycle-test-buffer-1"))
     ...
     (unwind-protect
         (progn
           (mapc (lambda (buf) (with-current-buffer buf (huecycle)))
                 buffer-access-list)
...
(ert-deftest huecycle-test-active-buffers-ordering ()
  "Test `huecycle--active-buffers' is properly set to reflect access ordering."
  ...
  (huecycle-with-test-env
   (let* ((buffer-1 (get-buffer-create "huecycle-test-buffer-1"))
     ...
     (unwind-protect
         (progn
           (mapc
            (lambda (buf) (with-current-buffer buf (huecycle)))
            buffer-access-list)
           (should (equal "huecycle-test-buffer-5"
                          (buffer-name (nth 0 huecycle--active-buffers))))
           (should (equal "huecycle-test-buffer-4"
                          (buffer-name (nth 1 huecycle--active-buffers))))
           (should (equal "huecycle-test-buffer-3"
                          (buffer-name (nth 2 huecycle--active-buffers))))
           (should (equal "huecycle-test-buffer-2"
                          (buffer-name (nth 3 huecycle--active-buffers))))
           (should (equal "huecycle-test-buffer-1"
                          (buffer-name (nth 4 huecycle--active-buffers)))))
       (mapc #'kill-buffer buffer-access-list)))
   :huecycle--interpolate-data
   (mapcar #'huecycle--init-interp-datum '(((foreground . default))))
   :huecycle--max-active-buffers 5)
